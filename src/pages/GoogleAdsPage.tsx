...
export const GoogleAdsPage: React.FC = () => {
  ...
  const { data: campaigns = [], error } = useQuery({
    queryKey: ['campaigns', 'google'],
    queryFn: () => fetchCampaigns('google'),
  });
  ...
  if (error) return (
    <div className="p-8 bg-red-50 text-red-700 rounded-2xl border border-red-100 flex items-center shadow-sm">
      ...
      Error al cargar campañas de Google: {(error as Error).message}
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Consola de Google Ads</h1>
          <p className="text-sm text-gray-500 mt-1">Monitorización de Search, Display y YouTube.</p>
        </div>
        <button 
          onClick={handleSync}
          disabled={isSyncing}
